...
#   ピン留めではない）
# このアカウントの manager だけに閉じる。user@.service に置くと、
# マシン上のすべてのセッションに委譲することになる
...
| ジョブの制限が `applied_limits` と一致しない | cpu が委譲されていない。3 章の確認 1 行で分かる。`/etc/systemd/system/user@<uid>.service.d/` の drop-in を置いて再起動する |
